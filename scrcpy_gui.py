...
from audio_output import (
    AudioOutputDevice,
    list_audio_outputs,
    set_process_audio_mute,
    set_process_audio_output,
)
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.active_serial = ""
        self.audio_outputs: list[AudioOutputDevice] = []
        self._phone_audio_process_ids: dict[QProcess, int] = {}
    ...
    def build_video_audio_group(self) -> QGroupBox:
        ...
        self.no_audio.stateChanged.connect(self.update_audio_output_control)
    ...
    def refresh_audio_outputs(self) -> None:
        """Refresh active Windows playback endpoints without delaying startup."""

        if not hasattr(self, "audio_output_combo"):
            return
        saved_endpoint = self.settings.value("audio_output", "", type=str)
        ...
        try:
            ...
            for device in devices:
                ...
                label = device.name + ("（系统当前默认）" if device.is_default else "")
                combo.addItem(label, device.endpoint_id)
            if saved_endpoint and saved_endpoint not in endpoint_ids and saved_endpoint != AUDIO_OUTPUT_PHONE:
                combo.addItem("已保存设备（当前不可用）", saved_endpoint)
            target = saved_endpoint
            target_index = combo.findData(target)
            combo.setCurrentIndex(target_index if target_index >= 0 else 0)
        finally:
            combo.blockSignals(False)
        self.update_audio_output_control()
        if not devices:
            self.log("未读取到 Windows 音频出口，scrcpy 将使用系统默认设备。")

    def selected_audio_output(self) -> AudioOutputDevice | None:
        endpoint_id = str(self.audio_output_combo.currentData() or "")
        if not endpoint_id or endpoint_id == AUDIO_OUTPUT_PHONE:
            return None
        return next(
            (device for device in self.audio_outputs if device.endpoint_id == endpoint_id),
            AudioOutputDevice(self.audio_output_combo.currentText(), endpoint_id),
        )

    def update_audio_output_control(self, *_args) -> None:
        if hasattr(self, "audio_output_combo"):
            self.audio_output_combo.setEnabled(not self.no_audio.isChecked())

    def on_audio_output_changed(self, *_args) -> None:
        if self._loading_settings:
            return
        self.save_settings()
        if self.no_audio.isChecked():
            return
        if self.audio_output_combo.currentData() == AUDIO_OUTPUT_PHONE:
            self.log("音频出口已选择：手机端（仅手机播放，Android 13+）。重新启动 scrcpy 后生效。")
            if self.scrcpy_process and self.scrcpy_process.state() != QProcess.ProcessState.NotRunning:
                self.set_status("手机端音频需要重新启动 scrcpy", False)
            return
        self.restore_phone_audio_for_running_processes()
        device = self.selected_audio_output()
        if not device:
            self.log("音频出口已设为跟随系统默认设备。")
            return
        if self.scrcpy_process and self.scrcpy_process.state() != QProcess.ProcessState.NotRunning:
            self.apply_audio_output_to_process(self.scrcpy_process, self.window_title_edit.text().strip() or "scrcpy")
        for process, title in self._extra_audio_processes():
            self.apply_audio_output_to_process(process, title)
        self.log(f"音频出口已选择：{device.name}")

    def restore_phone_audio_for_running_processes(self) -> None:
        """Restore computer playback when leaving the phone-only mode."""

        for process in list(self._phone_audio_process_ids):
            process_id = self._phone_audio_process_ids.pop(process, 0)
            if not process_id:
                continue
            set_process_audio_mute(process_id, False)

    def apply_phone_audio_to_process(self, process: QProcess, title: str, attempt: int = 0) -> None:
        """Keep audio on the phone and silence only this scrcpy process on Windows."""

        if (
            self.audio_output_combo.currentData() != AUDIO_OUTPUT_PHONE
            or self.no_audio.isChecked()
            or process.state() == QProcess.ProcessState.NotRunning
        ):
            return
        process_id = int(process.processId() or 0)
        if not process_id:
            return
        if attempt == 0:
            self.log(f"正在将 {title} 音频保留在手机端……")
        success, error = set_process_audio_mute(process_id, True)
        if success:
            self._phone_audio_process_ids[process] = process_id
            if attempt == 0:
                self.log(f"{title} 已切换为手机端播放，电脑端已静音")
            return
        if attempt < 8 and process.state() != QProcess.ProcessState.NotRunning:
            QTimer.singleShot(
                500,
                lambda p=process, t=title, a=attempt + 1: self.apply_phone_audio_to_process(p, t, a),
            )
        elif process.state() != QProcess.ProcessState.NotRunning:
            self.log(f"{title} 未能静音电脑端播放：{error}；手机仍会尝试保留播放。")

    def _extra_audio_processes(self):
        for index, process in enumerate(self.extra_scrcpy_processes, start=1):
            if process.state() != QProcess.ProcessState.NotRunning:
                yield process, f"独立窗口 {index}"

    def apply_audio_output_to_process(self, process: QProcess, title: str, attempt: int = 0) -> None:
        """Apply the selected endpoint after scrcpy obtains a Windows PID."""

        device = self.selected_audio_output()
        if not device or self.no_audio.isChecked() or process.state() == QProcess.ProcessState.NotRunning:
            return
        process_id = int(process.processId() or 0)
        if not process_id:
            return
        if attempt == 0:
            self.log(f"正在为 {title} 设置音频出口：{device.name}……")
        success, error = set_process_audio_output(process_id, device.endpoint_id)
        if success:
            if attempt == 0:
                self.log(f"{title} 已使用音频出口：{device.name}")
            return
        if attempt < 2:
            QTimer.singleShot(
                700,
                lambda p=process, t=title, a=attempt + 1: self.apply_audio_output_to_process(p, t, a),
            )
        else:
            self.log(f"{title} 音频出口设置失败：{error}；将继续使用系统默认设备。")
    ...
    def load_settings(self) -> None:
        ...
        audio_output = self.settings.value("audio_output", "", type=str)
        audio_output_index = self.audio_output_combo.findData(audio_output)
    ...
    def build_scrcpy_args(
        ...
        elif self.audio_output_combo.currentData() == AUDIO_OUTPUT_PHONE:
            # --audio-dup keeps playback on the phone. scrcpy rejects adding
            # --no-audio-playback because that disables the audio stream before
            # duplication can be initialized; the Windows session is muted
            # after scrcpy starts instead.
            args.extend(["--audio-source=playback", "--audio-dup"])
    ...
    def on_multi_scrcpy_started(self, _process: QProcess, title: str) -> None:
        ...
        if self.audio_output_combo.currentData() == AUDIO_OUTPUT_PHONE:
            self.apply_phone_audio_to_process(_process, title)
        else:
            self.apply_audio_output_to_process(_process, title)
    ...
    def on_multi_scrcpy_finished(self, process: QProcess, title: str, exit_code: int) -> None:
        ...
        self._phone_audio_process_ids.pop(process, None)
        self.update_multi_open_button()
    ...
    def on_scrcpy_started(self, title: str, args: list[str]) -> None:
        ...
        if self.scrcpy_process:
            if self.audio_output_combo.currentData() == AUDIO_OUTPUT_PHONE:
                self.apply_phone_audio_to_process(self.scrcpy_process, title)
            else:
                self.apply_audio_output_to_process(self.scrcpy_process, title)
    ...
    def on_scrcpy_finished(self, exit_code: int, _status) -> None:
        if self.scrcpy_process:
            self._phone_audio_process_ids.pop(self.scrcpy_process, None)
